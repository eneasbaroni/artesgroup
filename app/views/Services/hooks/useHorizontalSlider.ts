...
export const useHorizontalSlider = ({
  ...
  useEffect(() => {
    ...
    const wheelHandler = (e: WheelEvent) => {
      ...
      // Solo prevenir el scroll si NO estamos en la última ni en la primera slide
      const atLastSlide = indexRef.current === slidesCount - 1 && e.deltaY > 0;
      const atFirstSlide = indexRef.current === 0 && e.deltaY < 0;
      if (!atLastSlide && !atFirstSlide) {
        e.preventDefault();
      }
      ...
      if (wheelAccum.current > SLIDER_CONFIG.WHEEL_THRESHOLD) {
        // Move to next slide
        if (indexRef.current < slidesCount - 1) {
